# VSCode_mobile Design Documents Backup

移动端 VS Code 项目的架构、编码方案、协议草案、安全模型、路线图与 ADR。

- [备份来源与收录清单](BACKUP_SOURCE.md)
- 源仓库：<https://github.com/StarrySky7D4/VSCode_mobile>
- 源提交：`6ec5645541d6d426b81b32139d0331be185ee7bf`
- 许可：源仓库未发现根 `LICENSE`；详见 `LICENSE_STATUS.md`。

## 入口

- [Knowledge 索引](Knowledge/README.md)
- [完整项目编码方案](Knowledge/PROJECT_CODING_PLAN.md)
- [架构](Knowledge/ARCHITECTURE.md)
- [ADRs](Knowledge/ADRs/)
- [协议草案](protocol/ide/v1/)
