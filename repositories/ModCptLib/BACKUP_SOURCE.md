# Backup Source

> **Snapshot, not live documentation.** 本目录固定保存 2026-08-09 的 `master@add9e4024f8e10055508770aaf96c944aab6cceb`。截至 2026-08-26，活跃 `AutoSave@cd4f3b90cf70ad5e183df4eff121e04122458da1` 已领先 183 个提交；当前状态见 [CURRENT_STATUS.md](CURRENT_STATUS.md)。历史快照中的“当前”“已关闭”或协议支持声明不得直接外推到最新开发线。

- Source repository: <https://github.com/StarrySky7D4/ModCptLib>
- Source branch: `master`
- Source commit: `add9e4024f8e10055508770aaf96c944aab6cceb`
- Backup date: `2026-08-20`
- Source visibility at backup time: private
- Public backup authorization: explicitly granted by the repository owner on `2026-08-20`

## Included paths

- `Knowledge/ARCHITECTURE.md`
- `Knowledge/CONTENT_SECURITY.md`
- `Knowledge/PROTOCOL_LIFECYCLE.md`
- `Knowledge/PROTOCOL_REGISTRY.md`
- `Knowledge/README.md`
- `Knowledge/ROADMAP.md`
- `Knowledge/ROADMAP_SUPPLEMENT.md`
- `Knowledge/STRUCTURE.md`
- `Knowledge/designs/frb-api-evolution.md`
- `Knowledge/designs/groups-and-rooms-implementation.md`
- `Knowledge/designs/groups-and-rooms.md`
- `Knowledge/designs/identity-and-addressing-v2-implementation-plan.md`
- `Knowledge/designs/identity-and-addressing-v2.md`
- `Knowledge/designs/login-lifecycle-rewrite.md`
- `Knowledge/designs/m7-w33-operation-cancel-cleanup.md`
- `Knowledge/designs/m7-w33-privacy-safe-metrics-schema.md`
- `Knowledge/designs/m7-w33-public-error-contract.md`
- `Knowledge/designs/m7-w33-public-surface-inventory.md`
- `Knowledge/designs/m7-w33-release-version-rollback-adr.md`
- `Knowledge/designs/mailbox-first-v1.md`
- `Knowledge/designs/mock-removal.md`
- `Knowledge/designs/peer-identity-and-frb.md`
- `Knowledge/designs/relay-module-contract.md`
- `Knowledge/protocol-reference.md`
- `LICENSE`

Only selected design and licensing documents are included. Implementation code, credentials, build outputs, task dashboards, and unrelated audit logs are excluded.

## Backup-authored files

以下文件由本备份仓库维护，不是源提交中的文件：

- `README.md`
- `CURRENT_STATUS.md`
- `BACKUP_SOURCE.md`

为使选择性快照可独立浏览，少量指向未收录源文件的相对链接被改写为固定源提交的绝对 GitHub 链接；正文设计内容仍对应上述源提交。

