...
> - 第一轮：peer 去重修复（[`PEER_DEDUP_FIX.md`](PEER_DEDUP_FIX.md)）—— 临时层修补
...
| 归档文档 | 对应本文章节 | 状态 |
|----------|------------|------|
| [`PEER_DEDUP_FIX.md`](PEER_DEDUP_FIX.md) | §4 阶段 A | 被本文 §4 取代（原 §3.2 的 HashMap 索引改为遍历，见 §2.1 决策）；原 3 层防线细节保留备查 |
...
| [`DEFECT_ANALYSIS.md`](DEFECT_ANALYSIS.md) | §7 未来路径 | 缺陷编号（D1/D2/D3/D5/D6）对应本文 §7 |
| [mock-removal.md](mock-removal.md) | §7.1 近期 | Mock 移除独立指南 |
| [login-lifecycle-rewrite.md](login-lifecycle-rewrite.md) | §7.1（已完成） | 登录流程代码级实施版，已实施（参见 [登录重写日志](../logs/2026-07-07-auth-flow-rewrite.md)） |
